...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function PeakChip(props) {
      const w = windowOf(props.now);
      const peak = w.status === "peak";
      return React.createElement(
        "div",
        {
          className: "dsm-chip" + (peak ? " dsm-chip-peak" : ""),
          title: "北京时间工作日（周一至周五）高峰窗口：09:00-12:00、14:00-18:00（高峰价）；其余及周末为低峰（半价）",
        },
        React.createElement(
          "span",
    ...
    function CostChip(props) {
      ...
      const cur = windowOf(props.now).status === "peak" ? "peak" : "off";
      const curTier = cur === "peak" ? "高峰价" : "低峰价";
      const up = status.currentPricing[cur];
      const modelLines = status.perModel.map((m) => {
        return (
          m.family +
          "：输入 命中 " +
          fmtTokens(m.tokens.hit) +
          "/未命中 " +
          fmtTokens(m.tokens.miss) +
          " · 输出 " +
          fmtTokens(m.tokens.out) +
          " · 命中率 " +
          pct(m.hitRate) +
          " · ¥" +
          fmtCny(m.cost.total)
      ...
      const detail = [
        ...
        "当前模型：" + status.currentFamily + " · 轮数：" + props.turns + " · 请求：" + status.requests + " 次",
        "— 分模型消费 —",
      ]
        .concat(modelLines.length ? modelLines : ["（本会话暂无模型用量）"])
        .concat([
          "— 合计 —",
          "费用：高峰段 ¥" +
    ...
    function DsMeterRow(props) {
      ...
      return React.createElement(
        "div",
        { className: "dsm-row" },
        React.createElement(PeakChip, { now }),
        React.createElement(CostChip, { status, turns, now }),
